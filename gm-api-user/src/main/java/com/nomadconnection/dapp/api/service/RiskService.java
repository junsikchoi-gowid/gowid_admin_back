...
public class RiskService {
	...
	public ResponseEntity saveRisk(RiskDto riskDto, Long idxUser) {

		Risk risk = repoRisk.save(Risk.builder()
				.idxUser(idxUser)
				.date(LocalDate.now().format(DateTimeFormatter.BASIC_ISO_DATE))
				.ceoGuarantee(riskDto.isCeoGuarantee())
				.depositGuarantee(riskDto.getDepositGuarantee())
				.depositPayment(riskDto.isDepositPayment())
				.cardIssuance(riskDto.getCardIssuance())
				.ventureCertification(riskDto.isVentureCertification())
				.vcInvestment(riskDto.isVcInvestment())
		.build());
...
//		}
		...
		// MinStartCash
		// MinCashNeed
		if(riskDto.isVentureCertification() && riskDto.isVcInvestment()){
			risk.grade("A");
			risk.gradeLimitPercentage(10);
			risk.minStartCash((float) 100000000);
			risk.minCashNeed((float) 50000000);
		}else if(riskDto.isVentureCertification() && !riskDto.isVcInvestment()){
			risk.grade("B");
